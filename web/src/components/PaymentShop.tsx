...
export const PaymentShop: React.FC = () => {
  ...
  const handlePurchase = async (itemId: string, opcode: number, priceTon: string) => {
    ...
    try {
      ...
      const next = applyPurchase(itemId);
      setPurchases({ ...purchases, [itemId]: true });
  ...
  return (
    <div className="bg-gradient-to-br from-neon-dark/80 to-neon-dark/60 border border-neon-green/30 rounded-lg p-6 w-full">
      <h2 className="text-2xl font-bold text-neon-green mb-6 animate-glow">
        🛒 Покупки за TON
      </h2>
      ...
      <div className="grid grid-cols-1 gap-4">
        {SHOP_ITEMS.map((item) => (
          ...
          >
            ...
            >
              {purchases[item.id] ? "Unlocked" : loadingId === item.id ? "Оплата..." : "Купить"}
            </button>
          </div>
        ))}
      </div>
